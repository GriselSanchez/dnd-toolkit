...
const formatSpeed = (speed: ISpeed) => {
  const speedToArray = Object.entries(speed).map(([key, value]) => {
    ...
    const hoverProp = speed.hasOwnProperty('hover') && key === 'fly' && speed.hover ? '(hover)' : ''

    return `${speed.hasOwnProperty(key) ? `${StringUtils.formatToUpperFirstLetter(key)} ${value}ft.` : ''} ${hoverProp}`
  })
...
const formatSkills = (skills: ISkills) =>
  ...
  Object.entries(skills)
    .map(([key, value]) => `${StringUtils.formatToUpperFirstLetter(key)} +${value}`)
    .join(', ')
